% $Header: /u/gcmpack/manual/s_examples/held_suarez_cs/held_suarez_cs.tex,v 1.1 2005/08/01 22:58:25 jmc Exp $
% $Name:  $
...
using simple %Held and Suarez 
\cite{held-suar:94} forcing 
...
top of the atmosphere) follows the choice of \cite{held-suar:94} study, 
where normalized pressure coordinate ($\sigma_p$) was used. 
Note that without topography, the 2 coordinates systems are identical.
...
k_{v} & = & k_{f}~{\rm max}[0,(p^*/P^{0}_{s}-\sigma_{b})/(1-\sigma_{b})]
\\
\sigma_{b} & = & 0.7
\\
k_{f} & = & 1/86400 ~{\rm s}^{-1}
...
and $P^{0}_{s} = 10^5 {\rm Pa}$ is the pressure at the base of the atmospheric column.
